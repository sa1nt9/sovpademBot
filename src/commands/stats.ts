...
export const statsCommand = async (ctx: MyContext) => {
    ...
    if (existingUser) {
        try {
            ...
            const likesGiven = await prisma.profileLike.count({
                where: {
            ...
            const likesReceived = await prisma.profileLike.count({
                where: {
                    toProfileId: userId,
                    liked: true
                }
            ...
            const mutualLikes = await prisma.profileLike.count({
                where: {
                    fromProfileId: userId,
                    liked: true,
                    isMutual: true
            ...
            const dislikesGiven = await prisma.profileLike.count({
                where: {
                    fromProfileId: userId,
                    liked: false
                }
